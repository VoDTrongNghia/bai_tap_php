...
$row['price'] = (float)(
    ...
    /**
     * Get order by ID with user and items information
     */
    public static function getOrderById(int $orderId): array
    {
        $pdo = Database::getConnection();
        
        // Get order info with user details
        $query = "SELECT o.*, u.email as user_email, u.ho_ten as user_name, u.so_dien_thoai as user_phone
                  FROM don_hang o
                  LEFT JOIN nguoi_dung u ON o.user_id = u.id
                  WHERE o.id = ?";
        ...
        $itemsQuery = "SELECT ct.*, s.ten_sach as product_name, s.gia_goc as product_price
                       FROM chi_tiet_don_hang ct
                       LEFT JOIN sach s ON ct.id_sach = s.id
    ...
    public function updateStatus(string $newStatus): bool
    {
        ...
        // Cập nhật cả trang_thai và status để đồng bộ
        $stmt = $pdo->prepare("UPDATE don_hang SET trang_thai = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?");
        return $stmt->execute([$newStatus, $newStatus, $this->id]);
    }
    ...
    public function delete(): bool
    {
        ...
        try {
            ...
            $stmt = $pdo->prepare("DELETE FROM chi_tiet_don_hang WHERE id_don_hang = ?");
            $stmt->execute([$this->id]);
            
            // Delete order
            $stmt = $pdo->prepare("DELETE FROM don_hang WHERE id = ?");
    ...
    public function getDetails(): array
    {
        ...
        // Get order items
        $stmt = $pdo->prepare("SELECT * FROM chi_tiet_don_hang WHERE id_don_hang = ?");
        $stmt->execute([$this->id]);
        $items = $stmt->fetchAll(\PDO::FETCH_ASSOC);
