...
class BookRepository
{
    ...
    public function getDiscountBooks(): array
    {
        try {
            ...
            $stmt = $pdo->query("SELECT * FROM books WHERE phan_tram_giam > 0 ORDER BY phan_tram_giam DESC, id DESC LIMIT 4");
            $results = $stmt->fetchAll();
    ...
    public function searchByTitle(string $keyword): array
    {
        try {
            $pdo = Database::getConnection();
            $stmt = $pdo->prepare("SELECT * FROM books WHERE ten_books LIKE ? ORDER BY id DESC");
            $searchTerm = '%' . $keyword . '%';
    ...
    public function filterByCategory(string $category): array
    {
        try {
            $pdo = Database::getConnection();
            $stmt = $pdo->prepare("SELECT * FROM books WHERE id_danh_muc = ? ORDER BY id DESC");
    ...
    public function getAllCategories(): array
    {
        try {
            $pdo = Database::getConnection();
            $stmt = $pdo->query("SELECT DISTINCT id_danh_muc FROM books ORDER BY id_danh_muc");
