...
class BookRepository
{
    ...
    public function getDiscountBooks(): array
    {
        try {
            $pdo = Database::getConnection();
            // Get sach with discount (discount_percentage > 0)
            $stmt = $pdo->query("SELECT * FROM sach WHERE discount_percentage > 0 ORDER BY discount_percentage DESC, id DESC LIMIT 4");
            $results = $stmt->fetchAll();
    ...
    public function searchByTitle(string $keyword): array
    {
        try {
            $pdo = Database::getConnection();
            $stmt = $pdo->prepare("SELECT * FROM sach WHERE title LIKE ? ORDER BY id DESC");
            $searchTerm = '%' . $keyword . '%';
