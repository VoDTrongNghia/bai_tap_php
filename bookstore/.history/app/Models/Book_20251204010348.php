...
class Book
{
    ...
    public function getImageUrl(): string
    {
        ...
        // Ưu tiên dùng coverImage từ database
        if (!empty($this->coverImage)) {
            // If coverImage is already a full URL, return it
            if (filter_var($this->coverImage, FILTER_VALIDATE_URL)) {
                return $this->coverImage;
            }
            // If it's already includes uploads/products, just prepend baseUrl
            if (strpos($this->coverImage, 'uploads/products') !== false) {
                return $baseUrl . ltrim($this->coverImage, '/');
            }
            // If it's already includes assets/images/books, just prepend baseUrl (for backward compatibility)
            if (strpos($this->coverImage, 'assets/images/books') !== false) {
                return $baseUrl . ltrim($this->coverImage, '/');
            }
            // If it's a relative path starting with /, just prepend baseUrl
            if (strpos($this->coverImage, '/') === 0) {
                return $baseUrl . ltrim($this->coverImage, '/');
            }
            // Otherwise, assume it's in uploads/products
            return $baseUrl . 'uploads/products/' . $this->coverImage;
        }
